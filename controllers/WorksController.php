...
class WorksController extends Controller
{
    ...
    {
        $model = new Works();
        $fileModel = new UploadWorkForm();


        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate()){
                $fileModel->file = UploadedFile::getInstance($fileModel, 'file');
                if ($fileModel->upload($model->link, pathinfo($fileModel->file)['extension']) && $model->save()){
    ...
    public function actionDelete($id)
    {
        ...
        foreach ($files1 as $key => $value) {
            if (substr_count($value, $model->link . '.')  == 1){
                unlink('teamsWorks/' . $value);
                break;
            }
        }
